...
import com.android.videoviewerlib.view.SurfaceRenderView
import java.lang.ref.WeakReference

/**
 * ijkPlayer官方demo提供的长宽比转换工具类,获取到视频文件长宽后，计算合适的缩放比
 */
class MeasureHelper(view: View) {
    ...
    var measuredHeight = 0
        private set
    private var mCurrentAspectRatio = SurfaceRenderView.AR_ASPECT_FIT_PARENT
    val view: View?
        get() = mWeakView?.get()
    ...
    fun doMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        if (mCurrentAspectRatio == SurfaceRenderView.AR_MATCH_PARENT) {
            ...
            if (widthSpecMode == View.MeasureSpec.AT_MOST && heightSpecMode == View.MeasureSpec.AT_MOST) {
                ...
                when (mCurrentAspectRatio) {
                    SurfaceRenderView.AR_16_9_FIT_PARENT -> {
                        displayAspectRatio = 16.0f / 9.0f
                        if (mVideoRotationDegree == 90 || mVideoRotationDegree == 270) displayAspectRatio =
                            1.0f / displayAspectRatio
                    }
                    SurfaceRenderView.AR_4_3_FIT_PARENT -> {
                        displayAspectRatio = 4.0f / 3.0f
                        if (mVideoRotationDegree == 90 || mVideoRotationDegree == 270) displayAspectRatio =
                            1.0f / displayAspectRatio
                    }
                    SurfaceRenderView.AR_ASPECT_FIT_PARENT, SurfaceRenderView.AR_ASPECT_FILL_PARENT, SurfaceRenderView.AR_ASPECT_WRAP_CONTENT -> {
                ...
                when (mCurrentAspectRatio) {
                    SurfaceRenderView.AR_ASPECT_FIT_PARENT, SurfaceRenderView.AR_16_9_FIT_PARENT, SurfaceRenderView.AR_4_3_FIT_PARENT -> if (shouldBeWider) {
                        // too wide, fix width
                        width = widthSpecSize
                    ...
                    SurfaceRenderView.AR_ASPECT_FILL_PARENT -> if (shouldBeWider) {
                    ...
                    SurfaceRenderView.AR_ASPECT_WRAP_CONTENT -> if (shouldBeWider) {
                        // too wide, fix width
